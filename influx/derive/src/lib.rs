...
pub fn derive_to_line_protocol_entries(input: TokenStream) -> TokenStream {
    derive_to_line_protocol_entries_inner(input).unwrap_or_else(|e| e.into_token_stream())
}

fn derive_to_line_protocol_entries_inner(input: TokenStream) -> Result<TokenStream> {
    let parse = Parse::new(input)?;
    let (mut generator, attributes, body) = parse.into_generator();
    let attributes = attributes
        .get_attribute::<ContainerAttributes>()?
    ...
    match body {
        Body::Struct(body) => {
            ...
            .generate_to_line_protocol_entries(&mut generator)?;
        }
        Body::Enum(_body) => {
